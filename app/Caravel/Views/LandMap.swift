...
struct LandMap: View {
    ...
    // On iOS these are driven by touch (pinch + drag) — no scroll-wheel catcher.
    ...
    private func draw(_ ctx: GraphicsContext, _ size: CGSize, t: Double) {
        ...
        for lon in stride(from: -180.0, through: 180.0, by: 30) {
            ...
            for lat in stride(from: -80.0, through: 80.0, by: 4) {
                let pt = geo.project(GeoCoord(lat: lat, lon: lon), fit)
                if first { grat.move(to: pt); first = false } else { grat.addLine(to: pt) }
            }
        }
            ...
            if connected {
                ...
                if total > 1 {
                    ...
                    for k in 0..<dots {
                        ...
                        let pt = pointAt(frac, screen, lengths, total)
                        ctx.fill(Path(ellipseIn: CGRect(x: pt.x - 2.4, y: pt.y - 2.4, width: 4.8, height: 4.8)),
                                 with: .color(color))
                    }
                }
            }
    ...
    // to the chord, on the side away from `center` (the pin centroid).
    ...
    private func drawPin(_ ctx: GraphicsContext, at p: CGPoint, pin: MapPin, t: Double) {
        ...
        // label
        ctx.draw(Text(pin.label).font(.system(size: 11, weight: .semibold)).foregroundColor(.white),
