...
// password/account we only know the bundle + mode until the engine connects.
...
// Profiles is the on-disk profile store, shared between the app and the
// PacketTunnel extension via the App Group container. It mirrors the mac client's
// Profiles.swift but roots the store in the App Group (Shared.storeDir) so both
// processes see the same files (cloud-sync.md §2). Sidecar markers flag a bundle
// as cloud-synced (`<name>.synced`) or disabled (`<name>.disabled`).
enum Profiles {
    static var dir: URL { Shared.storeDir }
    ...
    static func deviceIDPath(_ name: String) -> URL { dir.appendingPathComponent("\(name).pharosid") }
    ...
    // details appear once the engine connects.
    ...
    // protoLabel lists the node's protocol(s) for display.
    private static func protoLabel(_ node: [String: Any]) -> String? {
        ...
        let names = protos.compactMap { $0["type"] as? String }.map { t -> String in
            switch t {
            case "amneziawg": return "AmneziaWG"
            case "xray", "xray-reality": return "XRay"
            default: return t
            }
        }
